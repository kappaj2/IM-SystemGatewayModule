...
@Component({
    selector: 'jhi-region-inventory',
    templateUrl: './region-inventory.component.html'
})
export class RegionInventoryComponent implements OnInit, OnDestroy {
regions: RegionInventory[];
        ...
        if (this.currentSearch) {
            this.regionService.search({
                query: this.currentSearch,
                    ...
                    (res: HttpResponse<RegionInventory[]>) => this.regions = res.body,
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
       }
        this.regionService.query().subscribe(
            (res: HttpResponse<RegionInventory[]>) => {
    ...
    trackId(index: number, item: RegionInventory) {
        return item.id;
    }
