...
@Component({
    selector: 'jhi-country-inventory',
    templateUrl: './country-inventory.component.html'
})
export class CountryInventoryComponent implements OnInit, OnDestroy {
countries: CountryInventory[];
        ...
        if (this.currentSearch) {
            this.countryService.search({
                query: this.currentSearch,
                    ...
                    (res: HttpResponse<CountryInventory[]>) => this.countries = res.body,
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
       }
        this.countryService.query().subscribe(
            (res: HttpResponse<CountryInventory[]>) => {
    ...
    trackId(index: number, item: CountryInventory) {
        return item.id;
    }
